    ...
    public class TavernManager : MonoBehaviour
    {
        ...
        private bool CreateTavernItem(string memberId)
        {
            ...
            if (buttonRect != null)
            {
                ...
                Debug.Log($"[TavernManager] 按钮anchor已设置为顶部中心: Min(0.5,1) Max(0.5,1)");
            }
            ...
            if (purchaseButton != null)
            {
                ...
                // 在初始化后设置按钮位置：放置在成员下方，间距30
                // 使用协程延迟一帧，确保RectTransform尺寸已正确计算
                StartCoroutine(SetButtonPositionDelayed(buttonInstance, memberInstance, 30f));
                // 订阅购买成功事件，购买后销毁实例和按钮
        ...
        private void PositionButtonBelowItem(GameObject buttonInstance, GameObject itemInstance, float gap)
        {
            ...
            {
                Debug.LogWarning("[TavernManager] 无法设置按钮位置：缺少RectTransform组件");
                return;
            }
            ...
            Debug.Log($"[TavernManager] 按钮位置已设置: 物品顶部={itemTop:F1}, 物品底部={itemBottom:F1}, 按钮Y={buttonY:F1}, 间距={gap}");
        }
    }
}
