...
## 自动保存场景插件 (Auto Save Scene Editor)

### 概述

自动保存场景插件是一个Unity编辑器工具，用于定期自动保存当前场景，防止意外丢失工作。插件在编辑器启动时自动加载，无需手动配置即可使用。

### 功能特性

- **自动保存**: 定期自动保存当前打开的场景
- **可配置间隔**: 支持自定义保存间隔（10秒到30分钟）
- **智能检测**: 自动检测场景是否有未保存的更改
- **播放模式保护**: 播放模式下不会自动保存，避免干扰
- **设置窗口**: 提供可视化设置窗口，方便配置
- **菜单快捷方式**: 提供菜单项快速访问功能
...
插件在Unity编辑器启动时自动加载，默认启用，保存间隔为5分钟（300秒）。

**无需任何配置即可使用！**

#### 2. 通过菜单访问

在Unity编辑器的菜单栏中，找到 `Tools -> 自动保存场景`：
...
1. **场景检查**: 只保存已打开的场景，未打开的场景不会被保存
2. **未保存场景**: 如果场景从未保存过，会弹出保存对话框让用户选择保存位置
3. **播放模式**: 播放模式下不会自动保存，避免干扰游戏运行
4. **场景切换**: 切换场景时会重置保存计时器，避免立即保存

### 工作原理

1. **初始化**: 编辑器启动时自动加载插件
2. **定时检查**: 使用 `EditorApplication.update` 定期检查是否到了保存时间
3. **场景保存**: 调用 `EditorSceneManager.SaveScene()` 保存场景
4. **配置持久化**: 使用 `EditorPrefs` 保存配置，重启编辑器后仍然有效

### 调试信息

所有日志信息都带有 `[AutoSaveScene]` 前缀，方便在Console中搜索和过滤。
...
1. **未保存场景**: 如果场景从未保存过，自动保存会弹出保存对话框，用户可以选择保存位置或取消
2. **播放模式**: 播放模式下不会自动保存，这是为了避免干扰游戏运行
3. **场景切换**: 切换场景时会重置保存计时器，新场景不会立即被保存
4. **配置持久化**: 所有配置都保存在 `EditorPrefs` 中，与项目无关，所有项目共享同一配置
5. **性能影响**: 插件使用轻量级检查机制，对编辑器性能影响极小

### 技术实现

#### 核心类

1. **AutoSaveSceneEditor** (`Assets/Editor/AutoSaveSceneEditor.cs`)
   - 主插件类，使用 `[InitializeOnLoad]` 特性自动加载
   - 使用 `EditorApplication.update` 定期检查
   - 提供菜单项和配置接口

2. **AutoSaveSceneSettingsWindow** (`Assets/Editor/AutoSaveSceneEditor.cs`)
   - 设置窗口类，继承自 `EditorWindow`
   - 提供可视化配置界面
   - 实时显示状态和倒计时
...
使用 `EditorPrefs` 存储配置：
- `AutoSaveScene_Enabled`: 是否启用（bool）
- `AutoSaveScene_Interval`: 保存间隔（float，秒）
- `AutoSaveScene_LastSaveTime`: 上次保存时间（float）
...
## 批量设置纹理Filter Mode插件 (Batch Set Texture Filter Mode Editor)

### 概述

批量设置纹理Filter Mode插件是一个Unity编辑器工具，用于批量将所有纹理资源的Filter Mode设置为Point（无过滤模式）。这对于像素艺术风格的2D游戏特别有用，可以保持像素的清晰度，避免纹理被过滤模糊。

### 功能特性

- **批量处理**: 一次性处理多个纹理文件
- **灵活搜索**: 支持指定搜索路径和是否包含子目录
- **进度显示**: 实时显示处理进度
- **安全确认**: 操作前提供确认对话框
- **快速模式**: 提供快速设置整个项目的选项
- **扫描功能**: 可以先扫描查看会处理哪些文件
...
在Unity编辑器的菜单栏中，找到 `Tools -> 批量设置纹理Filter Mode -> 设置为Point`，打开设置窗口。

#### 2. 使用设置窗口

1. **选择搜索路径**:
   - 默认搜索路径为 `Assets`
   - 可以手动输入路径或点击"选择文件夹"按钮选择
   - 路径必须是Assets目录下的相对路径

2. **设置选项**:
   - **包含子目录**: 是否递归搜索子目录中的纹理文件

3. **扫描纹理**:
   - 点击"扫描纹理"按钮，查看会处理哪些文件
   - 窗口会显示找到的纹理数量

4. **执行设置**:
   - **设置所有纹理为Point**: 处理当前搜索路径下的所有纹理
   - **设置整个项目为Point**: 处理整个Assets目录下的所有纹理

#### 3. 快速设置（菜单项）

在Unity编辑器的菜单栏中，找到 `Tools -> 批量设置纹理Filter Mode -> 快速设置所有纹理为Point`，可以直接设置整个项目的所有纹理。
...
**其他模式**:
- **Bilinear（双线性）**: 平滑过滤，适合3D游戏
- **Trilinear（三线性）**: 更平滑的过滤，适合3D游戏

### 工作原理

1. **文件搜索**: 扫描指定目录下的所有纹理文件
2. **导入器获取**: 获取每个纹理的 `TextureImporter`
3. **设置Filter Mode**: 将 `filterMode` 设置为 `FilterMode.Point`
4. **重新导入**: 调用 `SaveAndReimport()` 应用更改
5. **刷新资源**: 刷新资源数据库，更新Unity编辑器

### 调试信息

所有日志信息都带有 `[BatchSetTextureFilterMode]` 前缀，方便在Console中搜索和过滤。
...
1. **备份项目**: 建议在操作前备份项目，因为此操作会修改纹理的导入设置
2. **处理时间**: 处理大量纹理文件可能需要一些时间，请耐心等待
3. **重新导入**: 操作会触发纹理的重新导入，可能会暂时占用编辑器资源
4. **不可逆操作**: 此操作会覆盖原有的Filter Mode设置，请谨慎使用
5. **进度条**: 处理过程中可以点击进度条的"取消"按钮取消操作
6. **路径要求**: 搜索路径必须是Assets目录下的相对路径（如 `Assets/Art`）
...
**BatchSetTextureFilterModeEditor** (`Assets/Editor/BatchSetTextureFilterModeEditor.cs`)
- 继承自 `EditorWindow`，提供可视化设置窗口
- 使用 `TextureImporter` 修改纹理导入设置
- 使用 `Directory.GetFiles()` 搜索文件
- 使用 `EditorUtility.DisplayProgressBar()` 显示进度

#### 关键方法

- `FindTextureFiles()`: 查找所有纹理文件
- `SetAllTexturesFilterModeToPoint()`: 批量设置Filter Mode
- `ScanTextures()`: 扫描纹理文件（仅查看，不修改）

### 扩展建议

如果需要扩展功能，可以考虑：
- 支持设置其他Filter Mode（Bilinear、Trilinear）
- 支持按文件类型筛选（只处理特定格式）
- 支持撤销操作（记录修改历史）
- 支持批量设置其他纹理导入设置（压缩格式、最大尺寸等）
- 添加预览功能（显示修改前后的效果）